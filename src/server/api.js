...
app.post('/api/ai-coach', async (req, res) => {
    try {
        dotenv.config();
        const { message, history = [], currentPlan = null, runnerContext = {}, customApiKey, } = req.body;
        ...
        if (recentRuns.length > 0) {
            const totalKm = recentRuns.reduce((acc, r) => acc + (r.distance_km || 0), 0);
            const avgPace = Math.round(recentRuns.reduce((acc, r) => acc + (r.duration_seconds || 0), 0) / (totalKm || 1));
            ...
            const last5 = recentRuns.slice(0, 5).map((r) => {
...
6. Target paces must be realistic based on the runner's current fitness and logged runs. Paces in JSON must be in seconds per km (e.g. 6:30/km = 390).`;
        ...
        const isExplicitPlan = (() => {
            ...
            return (l.includes('make this as plan') ||
...
function sanitizePlan(parsed) {
    ...
    const workouts = (parsed.workouts || []).map((w, idx) => ({
    ...
    const totalKm = workouts.reduce((acc, w) => acc + (w.distanceKm || 0), 0);
    return {
        id: planId,
...
function generateAlgorithmicPlan(prompt, runnerContext) {
    ...
    if (runnerContext?.recentRuns && runnerContext.recentRuns.length > 0) {
        const totalDuration = runnerContext.recentRuns.reduce((acc, r) => acc + (r.duration_seconds || 0), 0);
        const totalKm = runnerContext.recentRuns.reduce((acc, r) => acc + (r.distance_km || 0), 0);
        if (totalKm > 0) {
