...
const ZONE_COLORS: Record<string, { bg: string; text: string; fill: string }> = {
  Extreme: { bg: 'bg-rose-500', text: 'text-rose-400', fill: '#F43F5E' },
  Anaerobic: { bg: 'bg-orange-500', text: 'text-orange-400', fill: '#F97316' },
  Aerobic: { bg: 'bg-emerald-500', text: 'text-emerald-400', fill: '#10B981' },
  'Fat Burning': { bg: 'bg-cyan-500', text: 'text-cyan-400', fill: '#06B6D4' },
  'Warm Up': { bg: 'bg-indigo-400', text: 'text-indigo-400', fill: '#818CF8' },
};
...
export const RunCharts: React.FC<RunChartsProps> = ({
  ...
  if (!hasPoints && !hasZones) {
    return (
      <div className="bg-[#1E1E1E] rounded-3xl p-8 border border-white/5 shadow-soft text-center my-4 space-y-2">
        <div className="w-12 h-12 rounded-full bg-[#FF5500]/15 text-[#FF5500] flex items-center justify-center mx-auto">
          <Activity className="w-6 h-6" />
        </div>
        <p className="text-sm font-bold text-white">No chart data extracted</p>
  ...
  return (
    <div className="space-y-4">
      ...
      {hasZones && (
        <div className="bg-[#1E1E1E] rounded-3xl p-4 sm:p-5 border border-white/5 shadow-soft space-y-3">
              ...
              return (
                <div key={idx} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-neutral-300">{z.name}</span>
                    <div className="space-x-2 font-mono text-[11px]">
                      {z.duration_seconds !== null && z.duration_seconds !== undefined && (
                        <span className="text-neutral-400">{formatDuration(z.duration_seconds)}</span>
                      )}
                      <span className="font-bold text-white">{z.percentage}%</span>
                    </div>
                  </div>
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-[#111111] border border-white/10 text-white text-xs px-2.5 py-1.5 rounded-xl shadow-lg font-mono">
      ...
      {hasPoints && (
        <div className="bg-[#1E1E1E] rounded-3xl p-4 sm:p-5 border border-white/5 shadow-soft">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold uppercase tracking-wider text-neutral-400 flex items-center">
              <Mountain className="w-3.5 h-3.5 text-cyan-500 mr-1.5" />
              Elevation (m)
            </span>
          </div>
