import type { RouteData, Split, ElevationPoint } from '../types/run';
 ...
 */
export function parseGpx(gpxString: string): RouteData | null {
  if (!gpxString || !gpxString.trim()) return null;
  ...
  try {
    ...
    if (typeof DOMParser !== 'undefined') {
      ...
      if (pointsNodeList.length === 0) return null;
      ...
      for (let i = 0; i < pointsNodeList.length; i++) {
        ...
        if (cadNode && cadNode.textContent) {
          ...
          if (!isNaN(parsedCad)) cad = parsedCad;
        }
    ...
    const coordinates: [number, number][] = rawPoints.map((p) => [p.lat, p.lng]);
    const elevations: number[] = rawPoints.map((p) => p.ele ?? 0);
    const timestamps: string[] = rawPoints.map((p) => (p.time ? p.time.toISOString() : ''));
...
function calculateSplits(points: RawTrackPoint[]): Split[] {
...
export function generateRouteSvgPath(
  coordinates: [number, number][],
  width: number = 80,
  height: number = 60,
  padding: number = 8
): { pathData: string; startPoint: [number, number]; endPoint: [number, number] } | null {
